...
import { Download, MessageCircle, Phone, RefreshCw, Search, UserRound, Users, WalletCards } from 'lucide-react'
...
export default function ClientsPage() {
  ...
  const exportCsv = () => {
    ...
    const rows: Array<Array<string | number>> = [
      ...
      ...filteredClients.map((client) => [client.name, client.phone, client.saleCount, Math.round(client.totalPurchases), Math.round(client.totalPaid), Math.round(client.totalDue), new Date(client.lastPurchase).toLocaleString('fr-SN')]),
    ]
    ...
    <div className="min-h-screen">
      ...
      <div className="space-y-4 p-3 sm:p-4 lg:space-y-6 lg:p-6">
        ...
        <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_180px_auto_auto]">
          <label className="relative block"><span className="sr-only">Rechercher un client</span><Search size={16} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[#6B7682]" /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nom ou téléphone…" className="h-11 w-full rounded-xl border border-[#2D7D7D]/[0.12] bg-white pl-10 pr-4 text-sm text-[#1A3636]" /></label>
          <select value={filter} onChange={(event) => setFilter(event.target.value as ClientFilter)} aria-label="Filtrer les clients" className="h-11 rounded-xl border border-[#2D7D7D]/[0.12] bg-white px-3 text-sm text-[#1A3636]"><option value="all">Tous les clients</option><option value="debt">Avec une dette</option></select>
          <button type="button" onClick={exportCsv} disabled={filteredClients.length === 0} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#2D7D7D]/[0.12] bg-white px-4 text-xs font-semibold text-[#2D7D7D] disabled:opacity-50"><Download size={14} /> Exporter</button>
          <button type="button" onClick={() => void loadClients(true)} disabled={refreshing} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#2D7D7D]/[0.12] bg-white px-4 text-xs font-semibold text-[#2D7D7D] disabled:opacity-50"><RefreshCw size={14} className={refreshing ? 'animate-spin' : ''} /> Actualiser</button>
        </div>
        ...
        {loading ? (
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{[1, 2, 3, 4, 5, 6].map((item) => <div key={item} className="skeleton h-40 rounded-2xl" />)}</div>
            ...
            {filteredClients.map((client) => {
              ...
              return (
                <Card key={client.id} className="p-4">
                  <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-[#1A3636]">{client.name}</p><p className="mt-0.5 text-xs text-[#6B7682]">{client.phone || 'Aucun téléphone'}</p></div><span className={`shrink-0 rounded-lg px-2 py-1 text-[10px] font-semibold ${client.totalDue > 0 ? 'bg-amber-500/10 text-amber-700' : 'bg-emerald-500/10 text-emerald-700'}`}>{client.totalDue > 0 ? 'Dette ouverte' : 'Soldé'}</span></div>
                  <div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl bg-[#F4F7FB] p-3"><p className="text-[10px] uppercase text-[#6B7682]">Achats</p><p className="mt-1 text-sm font-semibold text-[#1A3636]">{formatCurrencyCompact(client.totalPurchases)}</p></div><div className={`rounded-xl p-3 ${client.totalDue > 0 ? 'bg-amber-500/10' : 'bg-emerald-500/10'}`}><p className="text-[10px] uppercase text-[#6B7682]">Reste</p><p className={`mt-1 text-sm font-semibold ${client.totalDue > 0 ? 'text-amber-700' : 'text-emerald-700'}`}>{formatCurrencyCompact(client.totalDue)}</p></div></div>
                  <div className="mt-3 flex items-center justify-between text-xs text-[#6B7682]"><span>{client.saleCount} vente(s)</span><span>{formatDate(client.lastPurchase)}</span></div>
                  {client.totalDue > 0 && contactPhone && <div className="mt-3 grid grid-cols-2 gap-2 border-t border-[#2D7D7D]/[0.07] pt-3"><a href={`https://wa.me/${contactPhone}?text=${encodeURIComponent(reminder)}`} target="_blank" rel="noreferrer" className="flex min-h-10 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-3 text-xs font-semibold text-white"><MessageCircle size={14} /> WhatsApp</a><a href={`tel:+${contactPhone}`} className="flex min-h-10 items-center justify-center gap-2 rounded-xl border border-[#2D7D7D]/15 px-3 text-xs font-semibold text-[#2D7D7D]"><Phone size={14} /> Appeler</a></div>}
                </Card>
              )
            })}
          </div>
