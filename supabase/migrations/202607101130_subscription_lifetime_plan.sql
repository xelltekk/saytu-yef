-- Add a perpetual / lifetime subscription plan.
-- Business assumption for now: 149000 FCFA one-time payment, no monthly renewal.
...
ALTER TABLE public.profiles
  ...
  CHECK (subscription_plan IN ('free', 'starter', 'pro', 'enterprise', 'lifetime'));

ALTER TABLE public.subscription_requests
  DROP CONSTRAINT IF EXISTS subscription_requests_current_plan_check;

ALTER TABLE public.subscription_requests
  ADD CONSTRAINT subscription_requests_current_plan_check
  CHECK (current_plan IN ('free', 'starter', 'pro', 'enterprise', 'lifetime'));

ALTER TABLE public.subscription_requests
  DROP CONSTRAINT IF EXISTS subscription_requests_requested_plan_check;

ALTER TABLE public.subscription_requests
  ADD CONSTRAINT subscription_requests_requested_plan_check
  CHECK (requested_plan IN ('free', 'starter', 'pro', 'enterprise', 'lifetime'));
...
AS $$
  SELECT CASE
    WHEN p_requested_plan = p_current_plan AND p_requested_plan = 'lifetime' AND p_current_status IN ('past_due', 'expired', 'suspended', 'cancelled') THEN 'reactivation'
    WHEN p_requested_plan = p_current_plan AND p_requested_plan = 'lifetime' THEN 'activation'
    ...
    WHEN p_current_plan = 'starter' AND p_requested_plan IN ('pro', 'enterprise', 'lifetime') THEN 'upgrade'
    WHEN p_current_plan = 'pro' AND p_requested_plan IN ('enterprise', 'lifetime') THEN 'upgrade'
    WHEN p_current_plan = 'enterprise' AND p_requested_plan = 'lifetime' THEN 'upgrade'
    WHEN p_current_plan = 'free' AND p_requested_plan <> 'free' THEN 'activation'
    ELSE 'downgrade'
  END;
$$;
...
BEGIN
  ...
  IF p_requested_plan NOT IN ('free', 'starter', 'pro', 'enterprise', 'lifetime') THEN
...
BEGIN
  ...
  ELSIF p_action = 'activate' THEN
    ...
    next_period_end := CASE
      WHEN req.requested_plan IN ('free', 'lifetime') THEN NULL
